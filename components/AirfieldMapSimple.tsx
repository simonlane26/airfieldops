  ...
  useEffect(() => {
    const loadDiagramData = async () => {
      try {
        ...
        const response = await fetch('/api/diagrams/current');
        if (!response.ok) {
          throw new Error('Failed to load airport diagram');
        }

        const data = await response.json();

        // Merge aprons into taxiways array (aprons use same Taxiway type)
        const allTaxiways = [
          ...(data.taxiways || []),
          ...(data.aprons || [])
        ].map((t: any) => ({
          ...t,
          status: (t.status || 'open') as TaxiwayStatus
        }));

        const allRunways = (data.runways || []).map((r: any) => ({
          ...r,
          status: (r.status || 'open') as TaxiwayStatus
        }));

        setAirfieldStatus({
          taxiways: allTaxiways,
          runways: allRunways,
          workAreas: []
        });

        setLabelOffsets(data.label_offsets || {});

        // Store background image if present
        if (data.background_image) {
          setBackgroundImage(data.background_image);
        }
      } catch (error) {
        console.error('Error loading diagram:', error);
        setDiagramError(error instanceof Error ? error.message : 'Failed to load diagram');
      } finally {
      ...
      {diagramLoading && (
        <div className="flex items-center justify-center h-64">
          <div className="flex flex-col items-center gap-4">
            ...
            <p className="text-slate-400">Loading airport diagram...</p>
          </div>
        </div>
      )}
      ...
      {diagramError && !diagramLoading && (
        <div className="flex items-center justify-center h-64">
          <div className="flex flex-col items-center gap-4 text-center">
            ...
            <p className="text-amber-400">{diagramError}</p>
            <p className="text-slate-400 text-sm">Please check your airport assignment or contact an administrator.</p>
          </div>
        </div>
      )}
